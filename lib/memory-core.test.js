...
describe('updateMemory', () => {
  test('replaces content and resets confidence to 0.5', () => {
    const { storePath } = makeTempStore();
    const mem = createMemory({ type: 'pattern', content: 'old content', tags: ['test'] });
    addMemory(storePath, mem);
    confirmMemory(storePath, mem.id); // bump to 0.7
    const updated = updateMemory(storePath, mem.id, 'new content');
